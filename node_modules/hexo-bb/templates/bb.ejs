...
<% if(content) { %>
<article class="bbmessage">
  <div class="bbmessage_body">
    <p><%= content %></p>
  </div>
</article>
<% } %>
...
<script type="text/javascript">
  ...
  function urlToLink(str) {
    //增加发图片功能
    var re = /\bhttps?:\/\/(?!\S+(?:jpe?g|png|bmp|svg|gif|webp|gif))\S+/g;
    var re_forpic = /\bhttps?:\/\/.*?(\.gif|\.jpeg|\.png|\.jpg|\.svg|\.bmp|\.webp)/g;
  ...
  function getData(page = 0) {
    query.descending('createdAt').skip(page * 10).limit(<%= limit %>).find().then(function (results) {
      if (results.length == 0) {
        alert('之前没发表过说说了')
      } else {
        let resC = results;
